...
def path(listi,ts,ls):
    ...
    if len(listi)-2 == ls:
        ...
        k[key] = {gildi:[listi[ls][ts]+","+str(max(int(listi[ls+1][ts+1]), int(listi[ls+1][ts]))),str(ts)+","+str(ls)]}
        #print(k)
        return gildi
    else:
        s1 = path(listi,ts,ls+1)
        s2 = path(listi,ts+1,ls+1)
        print(str(ts)+","+str(ls)+":"+k[str(ts)+","+str(ls+1)][s1][1])
        print(str(ts)+","+str(ls)+":"+k[str(ts+1)+","+str(ls+1)][s2][1])
        gildi = int(listi[ls][ts]) + max(s1,s2)
        if s1 >s2:
            k[key] = {gildi:[listi[ls][ts]+","+k[str(ts)+","+str(ls+1)][s1][0],str(ts)+","+str(ls)+":"+k[str(ts)+","+str(ls+1)][s1][1]]}
        elif s2 >= s1:
            k[key] = {gildi:[listi[ls][ts]+","+k[str(ts+1)+","+str(ls+1)][s2][0],str(ts)+","+str(ls)+str(ls)+":"+k[str(ts+1)+","+str(ls+1)][s2][1]]}
        #print(k)
        return gildi
